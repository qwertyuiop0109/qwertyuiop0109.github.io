...
*/
	...
	"alertSending": "Відправка повідомлення",
	"alertSetCallTime": "Будь ласка, вкажіть час дзвінка",
	"alertFillRequired": "Заповніть обов'язкові поля",
	...
	"sendResendError": "Повідомлення вже було надіслано",
	"sendError": "Помилка",
...
//		"fields": "+Введите ваш телефон(+380)*, " + (window.MessForCallHunter&&(typeof window.MessForCallHunter == 'string') ? "-Тема [="+ IPchrSChr(window.MessForCallHunter) +"]*" : "-Тема (Ваш вопрос)*") + ", VIN-код автомобиля (Необязательно)",
		"fields": "+Введіть ваш телефон(+380)*, " + (window.MessForCallHunter&&(typeof window.MessForCallHunter == 'string') ? "-Тема [="+ IPchrSChr(window.MessForCallHunter) +"]*" : "-Тема (Ваше питання)*") + ", VIN-код автомобіля (Необов'язково)",
