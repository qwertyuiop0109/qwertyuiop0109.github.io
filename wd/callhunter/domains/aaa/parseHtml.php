...
//проверяем наличие полей на странице
				var checkedfields = ParceFeedFields(html,fullhtml,OneFeedConfig,OneSiteConfig,URLID,url,feedsfields[OneFeedConfig['parentFeed']]);
			...
			if (typeof GASvalues['keys'] == 'object') {
				for (var ii = 0; ii < GASvalues['keys'].length; ii++) {
					if (GASvalues['keys'][ii]) {
						keysnotemptyflg = 1;
						var feedkeys = ItemKeys(GASvalues['keys'][ii],shortWordsHash,GAS.keysmixtype,GAS.maxNodigaddkeyNum,GASvalues['fixedkeys'],GAS.notReqfixedkeys);
						if (feedkeys) {
...
*/
				...
				keysnotemptyflg = 1;
				var feedkeys = ItemKeys(GASvalues['keys'],shortWordsHash,GAS.keysmixtype,GAS.maxNodigaddkeyNum,GASvalues['fixedkeys'],GAS.notReqfixedkeys);
				if (feedkeys) {
...
*/
		...
		if (GASvalues['masskeys']) {
			...
			for (var iii = 0; iii < masskeys.length; iii++) {
				if (masskeys[iii]) {
					keysnotemptyflg = 1;
					var feedkeys = ItemKeys(masskeys[iii],shortWordsHash,GAS.masskeysmixtype,GAS.maxNodigaddkeyNum,GASvalues['fixedkeys'],GAS.notReqfixedkeys);
					if (feedkeys) {
				        	try {
							addkeystypegen[GAS.addkeystype](feedkeys,keys,dkeys,addkeys,GAS.keywordtypes);
						} catch (e) {
...
//			var adheaders = ItemHeaders(GASADvalues[GASADId]['header'],shortWordsHash,GASADvalues[GASADId]['paths'],GASADvalues[GASADId]['header2'],GAS[GASADId]['keyword'],GASADvalues[GASADId]['startdescription'],GASADvalues[GASADId]['beforeprice'],GASvalues['price'],GASADvalues[GASADId]['afterprice'],GASADvalues[GASADId]['description'],GASADvalues[GASADId]['afterdescription'],GAS[GASADId]['priceaddtype'],GAS['priceRound'],GAS[GASADId]['noPaths'],GAS[GASADId]['headerTo1Line'],GAS[GASADId]['afterdescrNeed'],GAS[GASADId]['requiredescr'],0);
			...
			if (adheaders) {
				...
				if (!(GAS[GASADId]['FromGAS']['keys']&&GAS[GASADId]['FromGAS']['fixedkeys']&&GAS[GASADId]['FromGAS']['masskeys']&&GAS[GASADId]['FromGAS']['keysmixtype']&&GAS[GASADId]['FromGAS']['masskeysmixtype']&&GAS[GASADId]['FromGAS']['notReqfixedkeys'])) {
					...
					if (typeof GASADvalues[GASADId]['keys'] == 'object') {
						for (var iii = 0; iii < GASADvalues[GASADId]['keys'].length; iii++) {
							if (GASADvalues[GASADId]['keys'][iii]) {
								keysnotemptyflg = 1;
								var feedkeys = ItemKeys(GASADvalues[GASADId]['keys'][iii],shortWordsHash,GAS[GASADId]['keysmixtype'],GAS.maxNodigaddkeyNum,GASADvalues[GASADId]['fixedkeys'],GAS[GASADId]['notReqfixedkeys']);
								if (feedkeys) {
						...
						if (GASADvalues[GASADId]['keys']) {
							keysnotemptyflg = 1;
							var feedkeys = ItemKeys(GASADvalues[GASADId]['keys'],shortWordsHash,GAS[GASADId]['keysmixtype'],GAS.maxNodigaddkeyNum,GASADvalues[GASADId]['fixedkeys'],GAS[GASADId]['notReqfixedkeys']);
							if (feedkeys) {
					...
					if (GASADvalues[GASADId]['masskeys']) {
						...
						for (var iii = 0; iii < masskeys.length; iii++) {
							if (masskeys[iii]) {
								keysnotemptyflg = 1;
								var feedkeys = ItemKeys(masskeys[iii],shortWordsHash,GAS[GASADId]['masskeysmixtype'],GAS.maxNodigaddkeyNum,GASADvalues[GASADId]['fixedkeys'],GAS[GASADId]['notReqfixedkeys']);
								if (feedkeys) {
							        	try {
										addkeystypegen[GAS.addkeystype](feedkeys,keys,dkeys,addkeys,GAS.keywordtypes);
									} catch (e) {
...
function ParceFeedFields(html,fullhtml,OneFeedConfig,OneSiteConfig,URLID,url,feedsfields) {
...
//обрезаем если вдруг длинный домен
							...
							var GASparams = ['url','mobileurl','keys','fixedkeys','masskeys','header','paths','header2','price','description','afterdescription'];
							GAS.GASparams = [];
							...
							for (var k in GAS) {
								...
								var GASADparams = ['header','paths','header2','keys','fixedkeys','masskeys','startdescription','description','beforeprice','afterprice','afterdescription'];
								GAS[k].GASADparams = [];
								...
								//keys проверяем на уровне объявлений (может отсутсвовать, если есть masskeys)
