	...
	for (var ii in checkedfields.checkedfields) {
		if (checkedfields.checkedfields[ii]['field_value']) {
			...
			OUTPUT += '<br>FIELD ' + checkedfields.checkedfields[ii]['field_id'] + ' ' + ii + '<br>EMPTY FIELD<br>';
						...
						if (checkedfields.referrer_field_name) {
...
function ParceFeedFields(html,fullhtml,OneFeedConfig,OneSiteConfig,URLID,url,referrer_url,referrer_field,feedsfields) {
...
//						if ((!fieldHash['field_Fun'])&&foundnotemptyfields) {
			...
			} else if (field_type === 'referrer_field_value') {

				field_value = referrer_field;
